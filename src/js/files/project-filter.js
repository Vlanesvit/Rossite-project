...
export function filter_case() {
	const filter = document.querySelector('.filter');
	if (filter) {

		const filterItem = document.querySelectorAll(".rs-case .filter__item"),
			checked = filter.querySelectorAll('input[type="checkbox"]'),
		...
		reset.addEventListener('click', function () {
			...
			filterChange(filterItem);
			checkedYear = []; checkedCity = []; checkedIndustry = [];
			checkedType = [];
			projectAdd.setAttribute('data-paged', 1);
			ajax_project('project_page');
		});
		checked.forEach(function (item) {
			item.addEventListener('input', function () {
				...
				projectAdd.setAttribute('data-paged', 1);
				ajax_project('project_page', checkedYear, checkedCity, checkedIndustry, checkedType)
			});
		});
		projectAdd.addEventListener('click', function (e) {
			...
			ajax_project('project_page', checkedYear, checkedCity, checkedIndustry, checkedType, true)
		});
	}
}